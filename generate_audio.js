// generate_audio.js
// Запускать: node --env-file=.env generate_audio.js [--collection=words|phrases] [--limit=N] [--voice=Kore] [--ids=1,2,3]
//
// Озвучивает армянские слова/фразы через Gemini TTS, заливает wav в Firebase Storage
// и проставляет audioUrl в соответствующем документе Firestore. Пропускает записи,
// у которых audioUrl уже есть — скрипт можно безопасно перезапускать частями.
//
// Бесплатный тариф gemini-2.5-flash-preview-tts даёт всего 3 запроса/минуту,
// поэтому между вызовами выдерживается пауза ~21с.
...
async function uploadAudio(collectionName, safeId, wavBuffer) {
  const storageRef = ref(storage, `audio/${collectionName}/${safeId}.wav`)
...
async function run() {
  ...
  for (const d of todo) {
    ...
    try {
      ...
      await updateDoc(doc(db, TARGET_COLLECTION, d.id), { audioUrl: url })
      done++
